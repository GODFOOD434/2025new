...
logger = logging.getLogger("test_db_transaction")
...
def test_db_transaction():
    ...
    try:
        ...
        try:
            ...
            # 添加到会话
            db.add(test_order)
            logger.info("Added order to session")
            
            # 刷新会话，获取 ID
            db.flush()
            logger.info(f"Flushed session, order ID: {test_order.id}")
            ...
            db.rollback()
            logger.error(f"Error testing transaction: {str(e)}")
            import traceback
